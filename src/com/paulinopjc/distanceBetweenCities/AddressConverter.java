...
public class AddressConverter{
	...
	private final static String URL = "http://maps.googleapis.com/maps/api/geocode/json";
	...
	public static void main(String[] args) throws IOException{
		...
		GoogleResponse res = new AddressConverter().convertToLatLong(city1);
		if(res.getStatus().equals("OK"))
		  {
		   for(Result result : res.getResults())
		   {
		    System.out.println("Lattitude of address is :"  +result.getGeometry().getLocation().getLat());
		    lat1 = Double.parseDouble(result.getGeometry().getLocation().getLat());
		    System.out.println("Longitude of address is :" + result.getGeometry().getLocation().getLng());
		    lng1 = Double.parseDouble(result.getGeometry().getLocation().getLat());
		...
		GoogleResponse res1 = new AddressConverter().convertToLatLong(city2);
		if(res1.getStatus().equals("OK"))
		  {
		   for(Result result : res1.getResults())
		   {
		    System.out.println("Lattitude of address is :"  +result.getGeometry().getLocation().getLat());
		    lat2 = Double.parseDouble(result.getGeometry().getLocation().getLat());
		    System.out.println("Longitude of address is :" + result.getGeometry().getLocation().getLng());
		    lng2 = Double.parseDouble(result.getGeometry().getLocation().getLat());
		    System.out.println("Location is " + result.getGeometry().getLocation_type());
		   }
		  }
		  else
		...
		Double distFinal = distCalc.distance(lat1, lng1, lat2, lng2, c);
		
		System.out.println("Distance between Cities is: " + Double.toString(distFinal));
		  
		  System.out.println("\n");
		  GoogleResponse res2 = new AddressConverter().convertFromLatLong("18.92038860,72.83013059999999");
